...
def qrcode(value, size='150x150'):
    ...
    query = urllib.urlencode({'chs':'{}x{}'.format(width, height), 'cht':'qr', 'chl':value, 'choe':'UTF-8', 'chld': 'h|0'})
    url = conditional_escape('{}?{}'.format(base, query))
    alt = conditional_escape(value)

    return mark_safe(u'<img class="qrcode" src="{}" width="{}" height="{}" alt="{}" />'.format(url, alt, width, height))
